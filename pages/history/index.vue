            ...
            <div class="container mt-2 px-0">
                ...
                <div class="cc-history-section pb-5">
                    ...
                    <div class="row mt-5 px-5 align-items-center" v-for="history in histories" :key="history.id">
                        ...
                        <div class="col-md-2 px-0" style="background: #F8F8F8;">
                            <div class="user-history px-4 text-center d-flex align-items-center">
		                        ...
		                        <NuxtLink :to="`/history/${history.id}`" class="cc-view-history font-metropolis" v-if="$auth.loggedIn">View Details</NuxtLink>
                    ...
                    <div class="col-lg-6 m-auto justify-content-between mt-5 text-center pagination">
                        <nuxt-link to="/history/page/2">Next Page »</nuxt-link>
                    </div>

                    <div class="text-center mt-5 py-5">
		                <NuxtLink to='/dashboard' class="back-btn font-metropolis" v-if="$auth.loggedIn">Dashboard</NuxtLink>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    async mounted() {
        ...
        if (!loggein) {
            ...
            //show
            try {
                const histories = await this.$axios.$get("history");
    ...
    watch: {
        search: function(query) {
            ...
            if(this.search) {
                ...
                try {
                    const histories = this.$axios.$get("history")
                    .then((response) => {
