			...
			<div class="d-flex px-0 py-0">
				<div class="sidebar-section  py-5" style="background: #021969;">
					<div class="sidebar sidebar-lg" id="sidebar">
						...
						<ul class="sidebar-links px-0 mt-5 pt-5" style="list-style-type: none;">
							...
							<div class="dashboard-user-data">
								<div class="d-flex cc-db-client-info mt-5 py-0 align-items-center" v-for="history in histories" :key="history.id">
									...
									<div style="flex: 0 0 20%; border-left: 1px solid #f1f1f1;" class="py-3 cc-total-amount">
										<div class="offset-1 offset-sm-4">
					...
					<div class="sidebar-section  py-2" style="background: #021969;">
						<div class="sidebar sidebar-lg" id="sidebar">
							...
							<ul class="sidebar-links px-0 mt-5 pt-5" style="list-style-type: none;">
								...
								<li class="sidebar-item mt-4">
									<a href="cc-dashboard.php"><img src="~assets/images/home.svg" /><span class=" mx-3 text-white font-metropolis ">Dashboard</span></a>
								</li>
								<li class="sidebar-item mt-4">
									<a href="cc-quotation.php"><img src="~assets/images/edit-quote-cc.svg" /><span class="mx-3 text-white font-metropolis">New Quotation</span></a>
								</li>
								<li class="sidebar-item mt-4">
									<a href="cc-drafts.php"><img src="~assets/images/draft-2.svg" />
										<span class="mx-3 text-white font-metropolis"> Draft</span></a>
								</li>

								<li class="sidebar-item mt-4">
									<a href="cc-history.php"><img src="~assets/images/history.svg" /><span class="mx-3 text-white font-metropolis">History</span></a>
								</li>
								<li class="sidebar-item mt-4">
									<a href="#"><img src="~assets/images/log-out-outline.svg" /><span class="mx-3 text-white font-metropolis">Log Out</span></a>
								</li>
							</ul>
						</div>
...
<script>
	...
	export default{
		...
		computed: {
			filterSearch: function() {
				return this.histories.filter((history) => {
					var clientname = JSON.parse(history.client_info);
					console.log(history);
					return Boolean;
					// return clientname.name.match(this.search); //not work
					// return history.ref_id.match(this.search); // works
				})
			}
		},
        ...
        computed: {
            ...
            prevPage() {
            	if (this.$route.params.num === "2") return "/dashboard";
                return "/dashboard/page/" + (+this.$route.params.num - 1);
            },
            nextPage() {
                return "/dashboard/page/" + (+this.$route.params.num + 1);
            },
		...
		methods:{
			...
			total_qoute(){
				try {
					const total = this.$axios.$get('total')
					.then((response) => {
			...
			total_user(){
				try {
					const total = this.$axios.$get('user')
					.then((response) => {
			...
			completed_qoute(){
				try {
					const completed = this.$axios.$get('completed')
					.then((response) => {
			...
			pending_qoute(){
				try {
					const pending = this.$axios.$get('pending')
					.then((response) => {
